 ...
 * 
 *  Modified by Piash - Added OpenCode Zen integration
 */

package com.tom.rv2ide.artificial.agents.opencodezen
...
/*
 * @author piashmsuf-eng @ https://github.com/piashmsuf-eng
 * OpenCode Zen AI Agent implementation
 */

class OpenCodeZen : AIAgent {
  ...
  override val providerName = "OpenCode Zen"

  companion object {
      fun registerAgent() {
          AIAgentRegistry.register("opencodezen", object : AIAgentRegistry.AgentFactory {
              override fun create(context: Context): AIAgent {
                  return OpenCodeZen()
              }
              
              override fun hasValidApiKey(): Boolean {
                  val key = ApiKey.getOpenCodeZenApiKey()
                  android.util.Log.d("OpenCodeZen", "hasValidApiKey check: ${key != null && key.isNotEmpty()}, key length: ${key?.length ?: 0}")
                  return key != null && key.isNotEmpty()
              }
              
              override fun getApiKey(): String? {
                  val key = ApiKey.getOpenCodeZenApiKey()
                  android.util.Log.d("OpenCodeZen", "getApiKey called, returning key of length: ${key?.length ?: 0}")
                  return key
              }
          })
      }
  ...
  override fun initialize(apiKey: String, context: Context) {
      try {
          ...
          var selectedModel = agents?.getAgent() ?: "opencode/claude-sonnet-4-5"
          
          // Ensure we're using a valid OpenCode Zen model
          if (!agents!!.isValidModelForProvider(selectedModel, "opencodezen")) {
              selectedModel = "opencode/claude-sonnet-4-5"
              agents?.setAgent(selectedModel)
              agents?.setProvider("opencodezen")
          }
        ...
        try {
          val key = apiKey
              ?: return@withContext Result.failure(
                  IllegalStateException("OpenCode Zen service not initialized")
              )
          ...
          val response = callOpenCodeZenAPI(key, fullPrompt)
  ...
  private fun callOpenCodeZenAPI(apiKey: String, prompt: String): String {
    android.util.Log.d("OpenCodeZen", "Starting API call to OpenCode Zen")
    
    val url = URL("https://api.opencode.ai/v1/chat/completions")
    ...
    try {
      ...
      android.util.Log.d("OpenCodeZen", "Request body: ${requestBody.toString()}")
      ...
      android.util.Log.d("OpenCodeZen", "Response code: $responseCode")
      
      if (responseCode != HttpURLConnection.HTTP_OK) {
        val errorStream = connection.errorStream?.bufferedReader()?.readText() ?: "Unknown error"
        android.util.Log.e("OpenCodeZen", "Error response: $errorStream")
        ...
        try {
          ...
          android.util.Log.e("OpenCodeZen", "Error type: $errorType, code: $errorCode, message: $errorMessage")
          
          // Identify specific error types
          when {
            responseCode == 429 || errorType.contains("rate_limit") || errorCode.contains("rate_limit") -> 
              throw com.tom.rv2ide.artificial.exceptions.RateLimitException("OpenCode Zen rate limit exceeded: $errorMessage")
            errorType.contains("insufficient_quota") || errorMessage.contains("quota") || errorMessage.contains("billing") -> 
              throw com.tom.rv2ide.artificial.exceptions.QuotaExceededException("OpenCode Zen quota exceeded: $errorMessage")
            errorType.contains("invalid_api_key") || errorCode.contains("invalid_api_key") -> 
              throw com.tom.rv2ide.artificial.exceptions.InvalidApiKeyException("Invalid OpenCode Zen API key: $errorMessage")
            responseCode == 401 -> 
              throw com.tom.rv2ide.artificial.exceptions.InvalidApiKeyException("OpenCode Zen authentication failed: $errorMessage")
            else -> 
              throw Exception("OpenCode Zen API error ($responseCode) - Type: $errorType, Code: $errorCode, Message: $errorMessage")
          }
          ...
          throw Exception("OpenCode Zen API error ($responseCode): $errorStream")
        }
      }
      
      val responseBody = connection.inputStream.bufferedReader().readText()
      android.util.Log.d("OpenCodeZen", "Success response received, length: ${responseBody.length}")
      ...
      throw Exception("No response from OpenCode Zen API")
    } catch (e: com.tom.rv2ide.artificial.exceptions.RateLimitException) {
      android.util.Log.e("OpenCodeZen", "Rate limit exception", e)
      throw e
    } catch (e: com.tom.rv2ide.artificial.exceptions.QuotaExceededException) {
      android.util.Log.e("OpenCodeZen", "Quota exceeded exception", e)
      throw e
    } catch (e: com.tom.rv2ide.artificial.exceptions.InvalidApiKeyException) {
      android.util.Log.e("OpenCodeZen", "Invalid API key exception", e)
      throw e
    } catch (e: java.net.SocketTimeoutException) {
      android.util.Log.e("OpenCodeZen", "Timeout exception", e)
      throw Exception("OpenCode Zen request timeout: ${e.message}")
    } catch (e: java.net.UnknownHostException) {
      android.util.Log.e("OpenCodeZen", "Network exception", e)
      throw Exception("Network error - cannot reach OpenCode Zen: ${e.message}")
    } catch (e: Exception) {
      android.util.Log.e("OpenCodeZen", "General exception", e)
      throw e
    } finally {
