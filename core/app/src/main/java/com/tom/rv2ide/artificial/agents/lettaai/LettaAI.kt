 ...
 * 
 *  Modified by Piash - Added Letta.ai integration
 */

package com.tom.rv2ide.artificial.agents.lettaai
 ...
 * Letta.ai AI Agent implementation
 */
...
class LettaAI : AIAgent {
  ...
  override val providerId = "lettaai"
  override val providerName = "Letta.ai"

  companion object {
      fun registerAgent() {
          AIAgentRegistry.register("lettaai", object : AIAgentRegistry.AgentFactory {
              override fun create(context: Context): AIAgent {
                  return LettaAI()
              }
              
              override fun hasValidApiKey(): Boolean {
                  val key = ApiKey.getLettaAIApiKey()
  ...
  override fun initialize(apiKey: String, context: Context) {
      try {
          ...
          if (!agents!!.isValidModelForProvider(selectedModel, "lettaai")) {
              selectedModel = "letta-free"
              agents?.setAgent(selectedModel)
              agents?.setProvider("lettaai")
          }
  ...
  override suspend fun generateCode(
      ...
      withContext(Dispatchers.IO) {
        try {
          ...
          val response = callLettaAIAPI(key, fullPrompt)
  ...
  private fun callLettaAIAPI(apiKey: String, prompt: String): String {
